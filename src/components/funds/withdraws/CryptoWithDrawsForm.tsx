...
const CHAINS: { value: WithdrawCryptoChain; label: string; icon: string }[] = [
  { value: "bsc", label: "BSC (BEP20)", icon: "🟡" },
  { value: "tron", label: "Tron (TRC20)", icon: "🔴" },
  { value: "eth", label: "Ethereum (ERC20)", icon: "🔷" },
...
export default function CryptoWithDrawsForm({ onBack }: CryptoWithdrawsFormProps) {
  ...
  const handleSendOtp = useCallback(async () => {
    ...
    await sendOtp({ amount, otp_type: "withdraw_crypto_otp" });
  }, [amount, sendOtp]);

  const handleChainChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    setChain(e.target.value as WithdrawCryptoChain);
    reset();
    resetOtp();
    setOtp("");
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = cryptoWithdrawSchema.safeParse({ amount, walletaddress, otp, termsAccepted });
    if (!result.success) {
    ...
    await submitWithdraw({ chain, amount, walletaddress, otp });
  };
  ...
  return (
    ...
    >
      ...
      <div className="mb-6 flex items-start justify-between">
        <div>
          <h2 className="text-2xl font-semibold text-slate-900 dark:text-white">Withdraw USDT</h2>
        ...
        <div className="flex gap-3">
          <div>
            ...
            <select
              ...
              className="rounded-md border border-slate-300 bg-transparent px-3 py-2 text-sm text-slate-800 focus:ring-1 focus:ring-indigo-500 focus:outline-none dark:border-slate-700 dark:text-white"
            >
              {CHAINS.map((c) => (
          ...
          <div>
            ...
            <select
              disabled
              className="cursor-not-allowed rounded-md border border-slate-300 bg-transparent px-3 py-2 text-sm text-slate-800 dark:border-slate-700 dark:text-white"
            >
              <option>USD</option>
            </select>
          </div>
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => {
              ...
              setFieldErrors((p) => ({ ...p, walletaddress: "" }));
            }}
            placeholder="Enter wallet address"
            className={`w-full rounded-lg border bg-white px-4 py-3 text-sm text-slate-800 focus:ring-1 focus:ring-indigo-500 focus:outline-none dark:bg-slate-800 dark:text-white ${fieldErrors.walletaddress ? "border-red-400" : "border-slate-300 dark:border-slate-700"}`}
          />
          {fieldErrors.walletaddress && (
            <p className="mt-1 text-xs text-red-500">{fieldErrors.walletaddress}</p>
          )}
        </div>
        ...
        <div className="text-sm leading-relaxed text-slate-500 dark:text-slate-400">
          <div className="flex items-start gap-3">
            ...
            <label htmlFor="acceptTerms" className="flex-1 cursor-pointer">
              ...
              <br />
              By clicking <strong>Withdraw</strong>, you agree that you accept the Terms &amp;
              Privacy Policy of Novotrend Ltd.
            </label>
          </div>
